...
\author[1]{Xin Zeng\footnote{Equally contributed authors}}
\author[1]{Ye Zheng$^*$}
...
%\newpage
\section{\Rpackage{permseqExample} Package}\label{section:permseqExamplePackage}
...
In each step, \Rpackage{permseq} will return an S4 \emph{Prior} class, which is described in section \ref{section:priorClass}. This S4 class summarizes the  analysis information and it will be updated in each step.\\\\
...
\begin{enumerate}
\item \texttt{hg19}: Name of the Bowtie index obtained as described in section \ref{subsection:bowtieIndex}, given as an argument in \Rpackage{permseq}.
...
In what follows, we will use the default options for the function arguments: \Rcode{fragL}, \Rcode{AllocThres}, \Rcode{capping}, \Rcode{vBowtie}, \Rcode{mBowtie}, \Rcode{pBowtie}, and \Rcode{save.files}. For more options, please refer to section \ref{section:moreOptions}.

\subsection{Creating prior module based on multiple data sets}\label{subsection:priorModule}
...
\subsubsection{Creating prior module with only DNase-seq data}\label{subsubsection:onlyDnase}
...
% vBowtie=2, mBowtie=99, pBowtie=8, save.files=TRUE)
%\end{verbatim}
This will create a \emph{Prior} S4 object which contains the main information that needs to be inherited to continue the analysis. For more details about the \emph{Prior} object, refer to section \ref{section:workFlow} or the R help manual of the package. \\
...
\subsubsection{Creating prior module with DNase-seq and Histone ChIP-seq data}\label{subsubsection:dnaseHistone}
...
The main slot that is different from the ones described in section \ref{subsubsection:onlyDnase} is \textbf{posLoc\_bychr}, which is a list by chromosome.\\ 
...
\subsubsection{Creating prior module with only Histone ChIP-seq data}\label{subsubsection:onlyHistone}
...
\item \Rcode{priorHistone\_multi}: This function generates prior from the selected histone ChIP-seq data using it as a numerical variable and applying the prior construction framework of Section \ref{subsubsection:onlyDnase}.
\end{enumerate}
...
The main slot that is different from the ones described in sections \ref{subsubsection:onlyDnase} and \ref{subsubsection:dnaseHistone} is 
...
\subsection{Generating priors}\label{subsection:generatePrior}

Using the \emph{Prior} object created in section \ref{subsection:priorModule}, the user can run the \Rcode{priorGenerate} function of \Rpackage{permseq} to generate priors.\\
...
For example, we can use the prior module from section \ref{subsubsection:onlyDnase}  and one ChIP\_partial dataset allowing maximum 6 Histone ChIP-seq datasets. 
...
This function updates the \emph{Prior} object created in section  \ref{subsection:priorModule}.\\ 
...
\subsection{Allocating multi-reads}\label{subsection:allocateMultireads}

Using the \emph{Prior} object created in section \ref{subsection:generatePrior}, the user can run the \Rcode{readAllocate} function of \Rpackage{permseq} to allocate the multi-reads by the prior built in the previous steps and get the final results.\\
...
\end{enumerate}
For example, we choose the output format of tagAlign and use the object created in section \ref{subsection:generatePrior}:
...
If no prior is provided, \emph{readAllocate} will be reduced to CSEM alignment. Users can refer to section \ref{subsection:csem} for allocating multi-reads without prior information (DNase and/or Histone ChIP-seq).
\section{\emph{Prior} Class}\label{section:priorClass}
\emph{Prior} is an S4 class containing information for ChIP-seq alignment.\\
It will be created in the first step of \Rpackage{permseq}, section \ref{subsection:priorModule}, and then updated with new information when prior is built, section \ref{subsection:generatePrior}, and reads are allocated, section \ref{subsection:allocateMultireads}.
\subsection{names()}
...
For example, in section \ref{subsection:allocateMultireads} we only use DNase-seq to build the prior on which we allocate the ChIP-seq reads. The summary for the resulting \emph{Prior} object from readAllocate will be:
...
% mBowtie=99, pBowtie=8, fragL=200)
%\end{verbatim}
If no \emph{Prior} object is provided, it is necessary to give the parameter value to \emph{bowtieIndex}, \emph{csemDir}, \emph{bowtieDir}, \emph{vBowtie}, \emph{mBowtie}, \emph{pBowtie}, and \emph{fragL} which  were set as \Rcode{NULL} in section \ref{subsection:allocateMultireads}.\\
...
The option \Rcode{save.files} in \Rcode{priorProcess} and \Rcode{priorHistone\_init} will allow the users to store intermediate files or not. The default is set as \Rcode{TRUE} which means saving the intermediate files.\\
...
\end{itemize}
By setting \Rcode{save.files=FALSE}, \Rpackage{permseq} will remove these intermediate files which will not be used in the latter proceduresr leaving only the BED file with allocated multi-reads and the file under the \Rcode{posLoc\_bychr} slot which is necessary in the next step.\\
The \emph{Prior} object generated will be the same as in section \ref{subsection:priorModule}.
