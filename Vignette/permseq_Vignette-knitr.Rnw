...
In what follows, we will use the default options for the function arguments: \Rcode{fragL}, \Rcode{AllocThres}, \Rcode{capping}, \Rcode{vBowtie}, \Rcode{mBowtie}, \Rcode{pBowtie}, and \Rcode{save.files}. For more options, please refer to section 6.
